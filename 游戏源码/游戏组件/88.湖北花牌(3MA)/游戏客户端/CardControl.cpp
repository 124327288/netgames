...
bool CCardResource::LoadResource()
{
	...
	//发动画牌的扑克
	m_ImageSendBottom.LoadResource(IDB_CARD_USER_BOTTOM,CARD_WIDTH,CARD_HEIGHT);
	m_ImageSendBottom1.LoadResource(IDB_CARD_USER_BOTTOM1,CARD_WIDTH,CARD_HEIGHT);
...
bool CCardResource::DestroyResource()
{
	...
	//发牌的扑克
	m_ImageSendBottom.DestroyResource();
	m_ImageSendBottom1.DestroyResource();
...
void CHeapCard::DrawCardControl(CDC * pDC)
{
	...
	for(int i=m_wMinusLastCount;i<m_wMinusHeadCount;i++)
	{	
...
//增加扑克数据
void CCardControl::AddCardData(BYTE cbCardData)
{
	m_CardItemArray[m_wCardCount].bShoot = false;
	m_CardItemArray[m_wCardCount].cbCardData=cbCardData;
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	//绘画准备
	//int nXExcursion=m_ControlPoint.x+(CountArray(m_CardItemArray)-m_wCardCount)*CARD_WIDTH;
	int nXExcursion=m_ControlPoint.x;

	//绘画扑克
	for (WORD i=0;i<m_wCardCount;i++)
	{
		//计算位置
		int nXScreenPos=nXExcursion+27*i;
		int nYScreenPos=m_ControlPoint.y+(((m_CardItemArray[i].bShoot==false)&&(m_wHoverItem!=i))?POS_SHOOT:0);
		...
		if(m_CardItemArray[i].cbCardData!=m_MagicCard)
		{
			...
			if(((cbCardData&MASK_COLOR)>>4)!=1)
			{
				g_CardResource.m_ImageSendBottom.DrawCardItem(pDC,cbCardData,nXScreenPos,nYScreenPos);
			}	
			else
			{
				g_CardResource.m_ImageSendBottom1.DrawCardItem(pDC,cbCardData,nXScreenPos,nYScreenPos);
			}
		}
	...
	if (m_CurrentCard.cbCardData!=0)
	{
		//计算位置
		//int nXScreenPos=m_ControlPoint.x+m_ControlSize.cx-CARD_WIDTH;
		int nXScreenPos=m_ControlPoint.x+m_ControlSize.cx-27;
