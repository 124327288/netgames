...
///////////////////////////////////////////////////////////////////
	BYTE cbNoWangCard[5],cbWangCard[2];
	ZeroMemory(cbNoWangCard,sizeof(cbNoWangCard));
	ZeroMemory(cbWangCard,sizeof(cbWangCard));

	BYTE cbWangCount=0;
	BYTE cbNoWangCount=0;
	for(BYTE i=0;i<5;i++)
	{
		if(cbCardData[i]==0x4E || cbCardData[i]==0x4F)
		{
			cbWangCard[cbWangCount]=cbCardData[i];
			cbWangCount++;
		}
		else
		{
			cbNoWangCard[cbNoWangCount]=cbCardData[i];
			cbNoWangCount++;
		}
	}
	...
	if(cbWangCount)
	{
		for(BYTE i=0;i<cbNoWangCount-1;i++)
		{
			for(BYTE j=i+1;j<cbNoWangCount;j++)
			{
				BYTE cbCard1=GetCardLogicValue(cbNoWangCard[i]);
				BYTE cbCard2=GetCardLogicValue(cbNoWangCard[j]);
				if((cbCard1+cbCard2)%10==0)
				{
					BYTE cbIndex=0;
					for(int k=0;k<cbNoWangCount;k++)
					{
						if(cbNoWangCard[k]!= cbNoWangCard[i] && cbNoWangCard[k] != cbNoWangCard[j])
						{
							cbCardData[cbIndex++]=cbNoWangCard[k];
						}
					}
					for(int k=0;k<cbWangCount;k++)
					{
						cbCardData[cbIndex++]=cbWangCard[k];
					}
					cbCardData[cbIndex++]=cbNoWangCard[i];
					cbCardData[cbIndex++]=cbNoWangCard[j];
		...
		for(BYTE i=0;i<cbNoWangCount-1;i++)
		{
			for(BYTE j=i+1;j<cbNoWangCount;j++)
			{
				...
				for(int k=0;k<cbNoWangCount;k++)
				{
					...
					{
						BYTE cbIndex=0;
						BYTE cbCard3=GetCardLogicValue(cbNoWangCard[k]);
						if((cbCard1+cbCard2+cbCard3)%10==0)
						{
							cbCardData[cbIndex++]=cbNoWangCard[i];
							cbCardData[cbIndex++]=cbNoWangCard[j];
							cbCardData[cbIndex++]=cbNoWangCard[k];

							for(BYTE n=0;n<cbNoWangCount;n++)
							{
								if( cbNoWangCard[n] != cbNoWangCard[i] && 
									cbNoWangCard[n] != cbNoWangCard[j] && 
									cbNoWangCard[n] != cbNoWangCard[k])
								{

									cbCardData[cbIndex++]=cbNoWangCard[n];
								}
							}

							for(int n=0;n<cbWangCount;n++)
							{///////////////////////////////
								cbCardData[cbIndex++]=cbWangCard[n];
							}
							return true;
						}	
					}
		...
		if(cbWangCount==1)
		{
			...
			for(BYTE i=0;i<cbNoWangCount-1;i++)
			{
				for(BYTE j=i+1;j<cbNoWangCount;j++)
				{
					BYTE cbCard1=GetCardLogicValue(cbNoWangCard[i]);
					BYTE cbCard2=GetCardLogicValue(cbNoWangCard[j]);
					if((cbCard1+cbCard2)%10>maxvalue)
					{
						maxvalue=(cbCard1+cbCard2)%10;
						BYTE cbIndex=0;
						for(int k=0;k<cbNoWangCount;k++)
						{
							if(cbNoWangCard[k]!= cbNoWangCard[i] && cbNoWangCard[k] != cbNoWangCard[j])
		...
		else if(cbWangCount==2)
		{
			cbCardData[0]=cbNoWangCard[0];
			cbCardData[1]=cbNoWangCard[1];
			cbCardData[2]=cbWangCard[0];
			cbCardData[3]=cbNoWangCard[2];
			cbCardData[4]=cbWangCard[1];
				...
				for(BYTE j=i+1;j<cbCardCount;j++)
				{
					...
					if(bHaveWang)
					{
						...
						if(cbTempValue>cbMaxValue)
						{
							...
							//BYTE *pTemp=cbCardData;
							//SortByWang(pTemp,3);
							//SortByWang(pTemp+3,2);
						}
					}
...
void CGameLogic::SortByWang(BYTE cbCardData[],BYTE cbCardCount)
{
	BYTE cbWangCount=0;
	for(BYTE i=0;i<cbCardCount;i++)
	{
	...
	if(!cbWangCount)return;

	for(BYTE i=0;i<cbCardCount-cbWangCount;i++)
	{
		if(cbCardData[i]==0x4E || cbCardData[i]==0x4F)
		{
			BYTE pos=cbCardCount-1;
			//if(cbCardData[pos]==0x4E || cbCardData[pos]==0x4F)
			//{
			...
			BYTE cbTemp=cbCardData[pos];
			cbCardData[pos]=cbCardData[i];
			cbCardData[i]=cbTemp;
