...
CGameServiceManager::CGameServiceManager(void)
{
	...
	lstrcpyn(m_GameServiceAttrib.szDataBaseName,TEXT("QPHoeDB"),CountArray(m_GameServiceAttrib.szDataBaseName));
	lstrcpyn(m_GameServiceAttrib.szDescription,TEXT("锄大地游戏服务组件"),CountArray(m_GameServiceAttrib.szDescription));
#ifndef VIDEO_GAME
	lstrcpyn(m_GameServiceAttrib.szClientModuleName,TEXT("Hoe.EXE"),CountArray(m_GameServiceAttrib.szClientModuleName));
	lstrcpyn(m_GameServiceAttrib.szServerModuleName,TEXT("HoeServer.DLL"),CountArray(m_GameServiceAttrib.szServerModuleName));
#else
	lstrcpyn(m_GameServiceAttrib.szClientModuleName,TEXT("HoeVD.EXE"),CountArray(m_GameServiceAttrib.szClientModuleName));
	lstrcpyn(m_GameServiceAttrib.szServerModuleName,TEXT("HoeVDServer.DLL"),CountArray(m_GameServiceAttrib.szServerModuleName));
#endif
...
bool __cdecl CGameServiceManager::RectifyServiceOption(tagGameServiceOption * pGameServiceOption)
{
	...
	{
		pGameServiceOption->lLessScore=__max(pGameServiceOption->lCellScore*200L,pGameServiceOption->lLessScore);
	}
