...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint Point)
{
	...
	if (m_OriPoint==Point)
	{
		tagCardItem * pNextCardItem=NULL;
		BYTE bTemp1=m_GameLogic.GetCardValue(pCardItem->bCardData);
		BYTE bTemp2=0;
		//判断下一个牌
		if((BYTE)dwCurrentIndex-m_bCount-1>=0)
		{
			pNextCardItem = &m_CardDataItem[(BYTE)dwCurrentIndex-m_bCount-1];
			bTemp2=m_GameLogic.GetCardValue(pNextCardItem->bCardData);
		}
		LONG lTempID;
		LONG lTimes=(LONG)dwCurrentIndex-m_bKeepCount;
		//判断连牌值是否相同且都弹起了
		for (lTempID=(LONG)dwCurrentIndex-1;lTempID>=0 && lTempID>=lTimes; lTempID--)
		{
			pNextCardItem=&m_CardDataItem[(BYTE)lTempID];
			BYTE bTemp3=m_GameLogic.GetCardValue(pNextCardItem->bCardData);
			if(pNextCardItem->bShoot==false || bTemp1!=bTemp3)break;
		}
		//非连点/没弹起/右边第一个牌/连牌值不相同
		if(m_OriPoint!=Point || !pCardItem->bShoot || (BYTE)dwCurrentIndex==0 
			|| (BYTE)dwCurrentIndex-m_bCount==0 || bTemp1!=bTemp2 )
		{
			//!(连牌值相同并都弹起了)
			if(!(lTempID==lTimes-1 || lTempID==-1))m_bCount=0;
			pCardItem->bShoot=!pCardItem->bShoot;
		}
		else
		{	
			m_bCount+=1;
			//ASSERT(m_bCount>0 && m_bCount<8);
			m_bKeepCount=m_bCount;
			if(((BYTE)dwCurrentIndex-m_bCount)>=0 && pCardItem->bShoot==true )
			{
				pCardItem=&m_CardDataItem[(BYTE)dwCurrentIndex-m_bCount];
				if(pCardItem->bShoot==false )pCardItem->bShoot=true;
			}
		}
	}
	else pCardItem->bShoot=!pCardItem->bShoot;
