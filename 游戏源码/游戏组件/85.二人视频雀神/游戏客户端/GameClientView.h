...
protected:
	...
	BYTE							m_cbUserAction[2];					//用户动作

	//位置变量
protected:
	CPoint							m_UserFlagPos[2];					//标志位置
	CPoint							m_UserListenPos[2];					//标志位置
...
protected:
	...
	//扑克控件
public:
	CHeapCard						m_HeapCard[2];						//堆立扑克
	CUserCard						m_UserCard;							//用户扑克
	CTableCard						m_TableCard[2];						//桌面扑克
	CWeaveCard						m_WeaveCard[2][4];					//组合扑克
	CDiscardCard					m_DiscardCard[2];					//丢弃扑克
...
public:
	...
	//视频组件
private:
	CVideoServiceControl 			m_DlgVedioService[2];				//视频窗口
