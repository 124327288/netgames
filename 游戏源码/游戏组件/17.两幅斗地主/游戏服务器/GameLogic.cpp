...
BYTE CGameLogic::GetCardType(const BYTE cbCardData[], BYTE cbCardCount)
{
	...
	case 4:	//天王炸
		{
			bool bMissileCard = true;
			for ( BYTE cbCardIdx = 0; cbCardIdx < cbCardCount; ++cbCardIdx )
			{
				if ( GetCardColor( cbCardData[ cbCardIdx ] ) != 0x40 ) 
				{
					bMissileCard = false;
					break;
				}
			}
			if ( bMissileCard ) return CT_MISSILE_CARD;
		}
	}
	...
	if ( 4 <= cbCardCount && cbCardCount <= 8 )
	{
		...
		if ((AnalyseResult.cbFiveCount==1)&&(cbCardCount==5)) return CT_BOMB_CARD;
		if ((AnalyseResult.cbSixCount==1)&&(cbCardCount==6)) return CT_BOMB_CARD;
		if ((AnalyseResult.cbSevenCount==1)&&(cbCardCount==7)) return CT_BOMB_CARD;
		if ((AnalyseResult.cbEightCount==1)&&(cbCardCount==8)) return CT_BOMB_CARD;
	}
...
bool CGameLogic::CompareCard(const BYTE cbFirstCard[], const BYTE cbNextCard[], BYTE cbFirstCount, BYTE cbNextCount)
{
	...
	//规则判断
	if ((cbFirstType!=cbNextType)||(cbFirstType!=CT_BOMB_CARD && cbFirstCount!=cbNextCount)) return false;
