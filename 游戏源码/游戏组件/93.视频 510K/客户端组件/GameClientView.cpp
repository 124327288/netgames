...
VOID CGameClientView::RectifyGameView(INT nWidth, INT nHeight)
{
	...
	//玩家界面元素设置
	//对家玩家-0表示
	m_ptFace[0].x=(nWidth-m_nXFace)/2;						//头像水平居中
	m_ptFace[0].y=5;										//头像距离上边5个像素
	m_ptName[0].x=m_ptFace[0].x+m_nXFace+10;				//名字比头像靠右10个像素
	m_ptName[0].y=m_ptFace[0].y;							//名字距离上边与头像相同
	...
	//玩家自己-2表示
	m_ptFace[2].x=(nWidth-m_nXFace)/2;						//头像水平居中
	m_ptFace[2].y=nHeight-m_nYTimer-5;						//头像下沿距离底边5个像素
	m_ptName[2].x=m_ptFace[2].x+m_nXFace+10;				//名字比头像靠右10个像素
...
VOID CGameClientView::DrawGameView(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		sprintf(pUserData->szName,"测试玩家%02d",i);
