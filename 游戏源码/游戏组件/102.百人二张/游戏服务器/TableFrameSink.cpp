...
#ifdef _DEBUG
#define TIME_PLACE_JETTON			16									//下注时间
#else
#define TIME_PLACE_JETTON			16									//下注时间
#endif
	...
	case GS_FREE:			//空闲状态
		{
			...
			//限制提示
			TCHAR szTipMsg[260];
			sprintf(szTipMsg,TEXT("本房间上庄条件为：%I64d,区域限制为：%I64d,玩家限制为：%I64d"),
		...
		if(pIServerUserItem!= NULL)
		{
			...
			if (((wBankerTime<=m_wBankerTime)&&(m_ApplyUserArray.GetCount()>1)) || lBankerScore<m_lApplyBankerCondition)
			{
				...
				if (lBankerScore<m_lApplyBankerCondition)
					...
					_sntprintf(szTipMsg,CountArray(szTipMsg),TEXT("[ %s ]做庄次数达到上限，强行换庄!"),pIServerUserItem->GetAccounts());
			...
			else if((wBankerTime<=m_wBankerTime)&&(m_ApplyUserArray.GetCount()==1)&&(lBankerScore>m_lApplyBankerCondition))
			{
				...
				TCHAR szTipMsg[128];
				_sntprintf(szTipMsg,CountArray(szTipMsg),TEXT("由于没有玩家申请坐庄，本局由[ %s ]继续坐庄!"),pIServerUserItem->GetAccounts());
				SendGameMessage(INVALID_CHAIR,szTipMsg);
			}
...
bool CTableFrameSink::LoadConfig()
{
	...
	{
		CTraceService::TraceString(TEXT("未找到游戏配置文件"),TraceLevel_Exception);
		return false;
	}
