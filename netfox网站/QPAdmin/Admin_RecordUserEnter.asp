    ...
    Select case Request("swhat")
        ...
        Case "4"
            IF IsDate(SqlCheckStr(Request("search"))) Then
                ...
                queryCondition=" EnterTime>='"&enterTime1&"' AND EnterTime<'"&DateAdd("d",1,enterTime1)&"'"
            End IF
        Case "5"
...
With Page
	...
	.Pkey="RecordID"						'主键
	.Field="UserID,Score,KindID,ClientIP,ServerID,EnterTime"	'字段
	.Table="RecordUserEnter"				'表名
...
<br><table width='100%'><tr>
    <td align='left'>您现在的位置：<a href="Admin_GameUserMoney.asp">金币管理</a>&gt;&gt;<a href="Admin_RecordUserEnter.asp">&nbsp;进入房间记录</a></td>
...
<td><table width='100%' border='0' align='center' cellpadding='2' cellspacing='1' class='border'>
          <tr class='title'>             
            ...
            <td width='200' align='center'><strong>进房时间</strong></td>
            <td width='138' height='21' align='center'><strong>IP地址</strong></td>
            <td width='200' height='21' align='center'><strong>IP所属</strong></td>
          </tr>
          <%          
          Dim UserID,index,GameName
            ...
            For i=0 To Ubound(rs,2)
                UserID = Rs(0,i) 
                GameName=GetKindNameByKindID(Rs(2,i))
          %>
          <tr class='tdbg' onmouseout="this.style.backgroundColor=''" onmouseover="this.style.backgroundColor='#BFDFFF'"> 
            ...
            <td width='138' align='center'><label title="KindID是 <%=Rs(2,i) %>"><%=GameName%></label></td>
                                
	        <td width='38' align='center'><%=Rs(4,i)%></td>
	        <td width='200' align='center'><%=Rs(5,i)%></td>
            <td width='138' align='center'><%=Rs(3,i)%></td>
            <td width='200' align='center'><span id="disp_ip_<%=index %>" title="<%=rs(3,i) %>">正在查询IP...</span><script type="text/javascript">setTimeout("ShowIP('disp_ip_<%=index %>', '<%=rs(3,i) %>')",500*<%=index %>);</script></td>
          </tr>
          <%
          index=index+1
          Next
...
<input name='search' type='text' id="search"  size='20' maxlength='30' />
  <select name="swhat" id="swhat">
    ...
    <option value="4">按进房时间(天)</option>
    <option value="6">按登录IP</option>
  </select> 
