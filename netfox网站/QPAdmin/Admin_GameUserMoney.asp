...
<tr class='tdbg'> 
      ...
      <td width='687' height='30'> &nbsp;&nbsp; <a href="Admin_GameUsermoney.asp">所有用户</a> | <a href="?usertype=4">富豪排名</a>| <a href="?usertype=1">比赛用户</a>| <a href="?usertype=3">按登陆次数排序</a>| <a href="?usertype=2">游戏管理员</a></td>
</tr>
</form>
</table>
<%
...
If request("usertype")<>"" Then
    select case request("usertype")
	    ...
	    case "4"
		    OrderStr=" (Score+InsureScore) DESC"
    End Select
End If
...
fields="UserID,Score,Score AS Revenue,InsureScore,WinCount,LostCount,DrawCount,FleeCount,PlayTimeCount,AllLogonTimes,LastLogonIP"
End IF
...
<br />
<table width='100%'><tr>
    <td align='left'>您现在的位置：<a href="Admin_GameUsermoney.asp">金币管理</a>&gt;&gt;<a href="Admin_GameUsermoney.asp">&nbsp;所有成员列表</a></td>
    ...
    <form name='myform' method='Post' action='?command=edit' onsubmit="return confirm('确定要执行选定的操作吗？');">
      ...
      <table width='100%' border='0' align='center' cellpadding='2' cellspacing='1' class='border'>
          <tr class='title'>
            <% IF Request("usertype")="4" Then %>              
                <td width='85' height='21' align='center'><strong>排名</strong></td>
            <% End IF %>
            <td width='108' align='center'><strong>用户名</strong></td>
            <td width='85' height='21' align='center'><strong>金币数</strong></td>
...
rs.Open sql,GameConn,1,3
If rs.Eof Then
    Response.Write "<div style='margin:50px auto;width:700px;text-align:center;'>玩家未进入金币房间游戏，没有找到玩家的金币资料！<br /><a href='javascript:history.back(-1);'>&lt;&lt; 返回上一页</a></div>"
    Exit Sub
End IF
%>
<form name='myform' action='?action=saveuserinfo&id=<%=Request("id") %>&page=<%=Request("page") %>' method='post'>
<table width='100%'><tr>
    <td align='left'>您现在的位置：<a href="Admin_GameUsermoney.asp?page=<%=Request("page") %>">金币管理</a>&gt;&gt;修改用户金币</td>
    <td align='right'></td>
</tr></table>
  <table width='100%' border='0' cellspacing='1' cellpadding='2' class='border'>
    <tr class='title'> 
      <td height=22 colspan=2 align='center'> <b>修改用户金币</b></td>
    </tr>
	<tr class='tdbg'> 
