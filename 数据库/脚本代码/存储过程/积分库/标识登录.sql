...
BEGIN
	...
	SELECT @LockKindID=KindID FROM QPTreasureDBLink.QPTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID
	...
	-- 金币上限
	IF @Score>=@ScoreLimit OR @GameGold>=@ScoreLimit OR @InsureScore>=@ScoreLimit
	BEGIN
		-- 解除锁定
		DELETE FROM QPTreasureDBLink.QPTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID AND KindID=@wKindID  AND ServerID=@wServerID 

		SELECT [ErrorDescribe]=N'您的积分或金币数目已经超出系统上限，请联系客户服务中心了解详细情况！'
		RETURN 3
	END
