...
const BYTE	CGameLogic::m_cbCardData[CELL_PACK]=
{
	...
	0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1A,0x1B,0x1C,0x1D,	//黑桃 A - K
...
bool CGameLogic::EfficacyThrowCard(const BYTE cbOutCardData[], BYTE cbOutCardCount, WORD wOutCardIndex, const BYTE cbHandCardData[GAME_PLAYER][MAX_COUNT], BYTE cbHandCardCount, tagOutCardResult & OutCardResult)
{
	...
	for (WORD i=0;i<GAME_PLAYER-1;i++)
	{
		...
		{
			if(cbHandCardData[wUserIndex][j]==0x00) continue;
			if(GetCardLogicColor(cbHandCardData[wUserIndex][j])==COLOR_NT||GetCardLogicColor(cbHandCardData[wUserIndex][j])!=GetCardLogicColor(cbOutCardData[0])) continue;
			if(GetCardValue(cbHandCardData[wUserIndex][j])==14||GetCardValue(cbHandCardData[wUserIndex][j])==13||GetCardValue(cbHandCardData[wUserIndex][j])==12) 
			{
				OutCardResult.cbCardCount=1;
				BYTE cbLessCardData=cbOutCardData[0];
				for(WORD k=0;k<cbOutCardCount;k++)
				{
					if (GetCardValue(cbOutCardData[k])<GetCardValue(cbLessCardData)) cbLessCardData=cbOutCardData[k];
				}
...
bool CGameLogic::EfficacyOutCard(const BYTE cbOutCardData[], BYTE cbOutCardCount, const BYTE cbFirstCardData[], BYTE cbFirstCardCount, const BYTE cbHandCardData[], BYTE cbHandCardCount)
{
	...
	//逼对
	if ((cbFirstCardType==CT_SAME_2)&&(cbOutLogicColor==cbFirstLogicColor)&&(cbOutCardType==CT_THROW_CARD)) return true;
...
void CGameLogic::UpdateSortRight()
{
	...
	*/
	case COLOR_HEI_TAO:		//黑桃
		{
			m_cbSortRight[COLOR_HONG_TAO>>4]=COLOR_RIGHT;
			break;
		}
	case COLOR_FANG_KUAI:	//方块
		{
			m_cbSortRight[COLOR_MEI_HUA>>4]=2*COLOR_RIGHT;
			break;
		}
	}
