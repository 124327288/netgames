...
void CGameClientDlg::OnGameOptionSet()
{
	...
	//CString str;
	//str.Format("是否播放地方话:%d",m_wPlayNativeSound);
	//AfxMessageBox(str);
	return;
}
...
bool CGameClientDlg::OnTimerMessage(WORD wChairID, UINT nElapse, UINT nTimerID)
{
	...
	case IDI_START_GAME:		//开始游戏
		{
			...
			if (nElapse<=5) 
			{
				//layGameSound(AfxGetInstanceHandle(),TEXT("GAME_WARN"));
	...
	case IDI_OPERATE_CARD:		//操作定时器
		{
			//托管判断
			if((nElapse<=TIME_OPERATE_CARD) && m_IsTuoGuan && IsLookonMode()==false)
			{
				//获取位置
			...
			if ((nElapse==0)&&(IsLookonMode()==false))
			{
				...
				////第一轮操作超时
				//if(m_bGameStart==false && wMeChairID == wMeChairID)
				//{
				//	OnCardOperate(WIK_TONG_GIVE_UP,0);
				//}			
				//动作处理
				if (wChairID==wMeChairID)
				{
...
//-------------还原打牌的流程过程-------------------------------------//
			...
			{
				BYTE cbFirstCurrentAction=pStatusPlay->cbFirstCurrentAction;
				WORD cbFirstCurrentUser=pStatusPlay->wFirstCurrentUser;

				//当首操作用户为庄用户，并且其操作为WIK_NULL时，表明无用户可操作，正常启动游戏
				if((cbFirstCurrentUser==m_wBankerUser) && (cbFirstCurrentAction==WIK_NULL))
				{
					m_bGameStart = true;
				...
				if ((m_bGameStart==false) && (cbFirstCurrentUser==GetMeChairID()))
				{
					//当前用户是庄用户并且能胡牌的话，则先胡牌
					if((GetMeChairID()==m_wBankerUser) && (cbFirstCurrentAction&WIK_CHI_HU)!=0)
					{
					...
					// 当用户有藏牌操作时，则显示用户藏牌提示
					else if((cbFirstCurrentAction&WIK_SI_TONG)!=0 || (cbFirstCurrentAction&WIK_WU_TONG)!=0)
					{
						ShowTongOperateControl(cbFirstCurrentAction,0);
					}
				...
				if((m_bGameStart==false) && (cbFirstCurrentUser!=GetMeChairID()))
				{
					SetGameTimer(cbFirstCurrentUser,IDI_OPERATE_CARD,TIME_OPERATE_CARD);
...
bool CGameClientDlg::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
	...
	if(/*m_bEnableSound*/IsEnableSound()==true)
	{
		CString szSoundName=_T("");
		szSoundName="Sound\\";
		szSoundName+="GAME_START.wav";
...
bool CGameClientDlg::OnSubSendCard(const void * pBuffer, WORD wDataSize)
{
    ...
    //-----此处是可以改进的----将发的牌设置到手中的处理过程---------

//如果藏牌玩家个数不为0,表明是第一轮的藏过程，直接将牌保存到发牌玩家手中,不进行其他处理
	if ((m_bGameStart==false))
	{
		WORD wViewChairID=SwitchViewChairID(m_wCurrentUser);
		m_bCardCount[m_wCurrentUser] += pSendCard->cbSendCount;

		if (wViewChairID!=1)
		{
			m_GameClientView.m_UserCard[wViewChairID].AddCardCount(pSendCard->cbSendCount);

			if((pSendCard->cbActionMask&WIK_CHI_HU)!=0)
			{
				SetGameTimer(pSendCard->wCurrentUser,IDI_OPERATE_CARD,TIME_OPERATE_CARD);
			}
		}
		//当前玩家
		else
		{
			//根据发牌的张数进行保存
			for (BYTE i=0; i<pSendCard->cbSendCount; i++)
			{
				//保存发的两张牌到索引中
				m_cbCardIndex[m_GameLogic.SwitchToCardIndex(pSendCard->cbCardData[i])]++;
			}

			//保存花牌到花牌索引数组中
			m_GameLogic.SaveHuaCard(pSendCard->cbCardData,pSendCard->cbSendCount,m_cbHuaCardIndex);
			SetWoindowData(pSendCard->cbCardData,pSendCard->cbSendCount);
...
//----故现将藏操作移动了操作结果处理的位置进行显示
			...
			//如果有胡牌操作，显示胡牌最佳组合
			if ((GetMeChairID()==m_wBankerUser) && (m_cbActionMask & WIK_CHI_HU)!=0 && IsLookonMode()==false)
			{
				//屏蔽掉胡牌后，设置界面
				m_cbActionMask = (m_cbActionMask & 0x7F);
				...
				BYTE hScore = pSendCard->cbHuScore;
				BYTE GoldCard = m_GameLogic.SwitchToCardData(pSendCard->cbGoldCard);
				
				GetHuCardWeave(pSendCard->cbCenterCard,pSendCard->cbWeaveKind,pSendCard->cbYaKou,HuCardweave);
				m_GameClientView.m_HuCardDlg.SetHuCardWeave(HuCardweave,hScore,GoldCard);
		...
		//扣除扑克
		m_cbLeftCardCount-=pSendCard->cbSendCount;
...
bool CGameClientDlg::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	if(/*m_bEnableSound*/IsEnableSound()==true)
	{
		...
		if (lScore>0L) 
		{
			...
			if (m_cbSex == 1)
			{
				//PlayGameSound(AfxGetInstanceHandle(),TEXT("b_满"));
				CString szSoundName=_T("");
			...
			{
				//PlayGameSound(AfxGetInstanceHandle(),TEXT("g_满"));
				CString szSoundName=_T("");
...
LRESULT CGameClientDlg::OnOutCard(WPARAM wParam, LPARAM lParam)
{
	...
	m_cbSex = GetUserData(GetMeChairID())->cbGender;
	//CString str;
	//str.Format("出牌时是否允许声音播放:%d",m_GameOption.m_bEnableSound);
	//AfxMessageBox(str);
	//播放声音
	if(/*m_bEnableSound*/IsEnableSound()==true)
	{
