...
-- 执行逻辑
BEGIN
	INSERT QPTreasureDBLink.QPTreasureDB.dbo.RecordPurchaseProp(SendUserID,RcvUserID,KindID,ServerID,CateID,CurCount,OnceCount,PachurseCount,SpendScore,ClientIP)
...
BEGIN
	...
	IF @dwKindID=0
		...
		IF GETDATE()<@MemberOverDate
		BEGIN
			SELECT UserID,CateID,KindID,PropCount FROM QPTreasureDBLink.QPTreasureDB.dbo.GameUserShelf WHERE UserID=@dwUserID AND UsedFlag=0
			UNION
	...
	ELSE
		...
		IF GETDATE()<@MemberOverDate
		BEGIN
			SELECT UserID,CateID,KindID,PropCount FROM QPTreasureDBLink.QPTreasureDB.dbo.GameUserShelf WHERE UserID=@dwUserID AND UsedFlag=0 AND (KindID=@dwKindID OR KindID=0)
			UNION
			SELECT UserID,CateID,KindID,PropCount FROM @tbMember
		END
		ELSE
...
BEGIN
	...
	IF @dwPropCount<=0
	BEGIN
		DELETE FROM QPTreasureDBLink.QPTreasureDB.dbo.GameUserShelf WHERE UserID=@dwUserID AND KindID=@dwKindID AND CateID=@dwCateID
	END
	ELSE
	BEGIN
		UPDATE QPTreasureDBLink.QPTreasureDB.dbo.GameUserShelf SET PropCount=@dwPropCount,UsedFlag=0,ClientIP=@strClientIP WHERE UserID=@dwUserID AND KindID=@dwKindID AND CateID=@dwCateID

		IF @@ROWCOUNT=0
		BEGIN
			INSERT QPTreasureDBLink.QPTreasureDB.dbo.GameUserShelf(UserID,CateID,KindID,PropCount,ClientIP) 
			VALUES(@dwUserID,@dwCateID,@dwKindID,@dwPropCount,@strClientIP)
		END
	END

	-- 写记录
	INSERT QPTreasureDBLink.QPTreasureDB.dbo.RecordUserProp (UserID,KindID,CateID,PropCount,ClientIP)
...
-- 执行逻辑
BEGIN
	SELECT [ID],[Name],Price,SendUserCharm,RcvUserCharm,Discount FROM QPTreasureDBLink.QPTreasureDB.dbo.FlowerCate WHERE Nullity=0
...
BEGIN
  ...
  FROM QPTreasureDBLink.QPTreasureDB.dbo.[GameShopCate] WHERE  Nullity=0
