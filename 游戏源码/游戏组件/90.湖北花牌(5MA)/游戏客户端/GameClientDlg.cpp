...
bool CGameClientDlg::OnTimerMessage(WORD wChairID, UINT nElapse, UINT nTimerID)
{
	...
	case IDI_OPERATE_CARD:		//操作定时器
		{
			//托管判断
			if((nElapse<=TIME_OPERATE_CARD-1) && m_IsTuoGuan && IsLookonMode()==false)
			{
				//获取位置
				...
				if (wChairID==wMeChairID)
				{
					...
					if (m_wCurrentUser == wMeChairID && (m_cbSendCardFinish==true))
					{
						BYTE cbCardData = 0;
						for(int i = 8; i >= 0; i--)
						{
...
//this->InsertSystemString("启动了发牌动画");
		//启动发牌动画
...
//进行藏牌处理
	if ((pOperateResult->cbOperateCode & WIK_SI_TONG) != 0 || (pOperateResult->cbOperateCode & WIK_WU_TONG)!=0 || (pOperateResult->cbOperateCode & WIK_TONG_GIVE_UP)!=0 )
	{
		...
		if (m_bGameStart == false)
		{
			...
			{
				//CString str;
				//str.Format("操作码为:%x",pOperateResult->cbCurrentAction);
				//str.Append("还有藏牌");
				//this->InsertSystemString(str);
...
//----故现将藏操作移动了操作结果处理的位置进行显示
			...
			//在此要注意，已将用户能进行的藏牌操作移动用户操作返回处进行处理了
		}
