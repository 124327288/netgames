...
/////////////////////////////////////////////////////////
	bool                m_bTongCard;                             //指示到达的牌是为藏牌操作还是自摸牌操作
	BYTE              m_cbSendCount;                             //每次发送的牌张数
	BYTE				m_cbSendCardData[2];					 //扑克数据
	BYTE              m_cbHuScore;                               //丫口的值
	BYTE              m_cbGoldCard;                              //丫口的值
	BYTE              m_cbYaKou[2];                              //丫口的值
	BYTE              m_cbCenterCard[8];                         //8种组合的操作类型
	BYTE              m_cbWeaveKind[8];                          //8种组合的操作类型
	BYTE              m_cbFirstCurrentUser;
	BYTE              m_cbFirstCurrentAction;                    //当前用户所对应的操作

	bool              m_cbSendCardFinish;                        //用户发牌动画是否结束
...
protected:
	...
	//设置用户手中的牌
	bool AddUserHandCard(BYTE cbCardData ,BYTE cbDataCount);
	//辅助函数
protected:
...
protected:
	...
	//发送一张牌动画结束
	LRESULT OnSendOneCardFinish(WPARAM wParam, LPARAM lParam);
	//发送牌动画结束
