...
protected:
    int m_nFishCount;
    DWORD m_dwFishTime;						              //开始时间

    int m_nScene;                               // 那个场景
    RoleNetObject m_RoleObject[GAME_PLAYER];
    std::map<int, FishNetObject*> m_FishObject;
...
protected:
    bool OnBuyBullet(IServerUserItem * pIServerUserItem, int nBulletStyle, int nBulletCount);
    bool OnFire(IServerUserItem * pIServerUserItem, CMD_C_FIRE *fire);
    bool OnCapture(IServerUserItem * pIServerUserItem, CMD_C_CAPTURE *capture);
    bool OnChangeCannon(IServerUserItem * pIServerUserItem, CMD_C_CHANGE_CANNON *change);
