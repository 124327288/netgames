...
CSkinImage				CCardControl::m_ImageLBackCard;					//背景图片
CSkinImage				CCardControl::m_ImageRBackCard;					//背景图片
//////////////////////////////////////////////////////////////////////////
...
CCardControl::CCardControl()
{
	...
	if (m_bLoad==false)
	{
		...
		m_ImageLBackCard.SetLoadInfo(IDB_HORIZONTIAL_CARD_BACK_L,AfxGetInstanceHandle());
		m_ImageRBackCard.SetLoadInfo(IDB_HORIZONTIAL_CARD_BACK_R,AfxGetInstanceHandle());
		...
		CImageHandle CardImageHandle1(&m_ImageRBackCard);
		m_RightCardSize.cx = m_ImageRBackCard.GetWidth() ;
		m_RightCardSize.cy = m_ImageRBackCard.GetHeight() ;
	}
...
void CCardControl::RectifyControl()
{
	...
	else
	{
		...
		ControlSize.cx=m_ImageLBackCard.GetWidth();
		ControlSize.cy=(dwCardCount>0)?(m_ImageLBackCard.GetHeight()+(dwCardCount-1)*m_dwCardVSpace):0;
	}
...
{//获取鼠标在牌控件上的第几张牌
	if (m_bHorizontal==true)
	{
		...
		//数据验证
		ASSERT(dwCardIndex>=0 && dwCardIndex<=HAND_CARD_COUNT) ;
...
void CCardControl::OnPaint()
{
	...
	//加载位图
	CImageHandle CardImageHandle1(&m_ImageRBackCard);
	CImageHandle CardImageHandle2(&m_ImageLBackCard);
	...
	for (INT i=0;i<m_CardDataItem.GetCount();i++)
	{
		...
		//绘画扑克
		if(enRight==m_enSide)
			m_ImageRBackCard.BitBlt(BufferDC,dwXScreenPos,dwYScreenPos,m_RightCardSize.cx,m_RightCardSize.cy,0,0);
		else if(enLeft==m_enSide)
			m_ImageLBackCard.BitBlt(BufferDC,dwXScreenPos,dwYScreenPos,m_RightCardSize.cx,m_RightCardSize.cy,0,0);
		else
...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint Point)
{
	...
	if (dwCurrentIndex!=m_dwCurrentIndex) return;//如果在牌A按下鼠标后移动到B再放开鼠标则无效
