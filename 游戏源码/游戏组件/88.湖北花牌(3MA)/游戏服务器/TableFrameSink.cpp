	...
	case GS_MJ_HAI_DI:	//海底状态
		{
			...
			//三家用户总共的四藏与五藏的总次数
			StatusPlay.bSiTongAllCount=0;
			StatusPlay.bWuTongAllCount=0;
			for (BYTE i=0; i<GAME_PLAYER; i++)
			{
				StatusPlay.bSiTongAllCount+=m_SiTongCount[i];
				StatusPlay.bWuTongAllCount+=m_WuTongCount[i];
			}
			//还剩下牌的总数目
			StatusPlay.cbLeftCardCount=m_cbLeftCardCount;
