...
#define IDC_JETTON_BUTTON_5000000	219								//按钮标识
#define IDC_JETTON_BUTTON_1000000   220
...
#define IDC_CONTROL_BUTTON			214									//按钮标识
...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btControl.Create(TEXT("。"),WS_CHILD,rcCreate,this,IDC_CONTROL_BUTTON);
...
void CGameClientView::RectifyGameView(int nWidth, int nHeight)
{
	...
	int nXPos = nWidth/2-160+17;
	int nSpace = 1;

	DeferWindowPos(hDwp,m_btJetton100,NULL,nXPos,nYPos,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btJetton1000,NULL,nXPos + nSpace + rcJetton.Width() * 1,nYPos,0,0,uFlags|SWP_NOSIZE);
	...
	DeferWindowPos(hDwp,m_btBankerStorage,NULL,nWidth/2-255-20-68+25+10,nHeight/2+190,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBankerDraw,NULL,nWidth/2-315-20-68+25+10,nHeight/2+190,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBank,NULL,nWidth/2+290-20,nHeight/2-340+74,0,0,uFlags|SWP_NOSIZE);


#endif

	DeferWindowPos(hDwp,m_btControl,NULL,nWidth/2-395+30+20-10,nHeight/2+155-60+30+70-30,3*2,3*2,uFlags);

	//结束移动
	EndDeferWindowPos(hDwp);

#ifdef _DEBUG
	m_btControl.ShowWindow(SW_SHOW);
#endif
	return;
}
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	m_ImageTimeFlag.BitBlt(pDC->GetSafeHdc(), nWidth/2-230+10, nHeight/2+195, nTimeFlagWidth, m_ImageTimeFlag.GetHeight(),m_cbTimeIndex * nTimeFlagWidth, 0);
        
	//最大下注
	DrawNumberString(pDC,GetMaxOddScore(),nWidth/2+30+5, nHeight/2 + 80, true);
	DrawNumberString(pDC,GetMaxEvenScore(),nWidth/2+30+5, nHeight/2 + 110, true);
	...
	if ( m_wCurrentBankerChairID != INVALID_CHAIR )
	{
		...
		if ( pUserData )
		{
			...
			//庄家总分
			TCHAR strBankerTotalScore[16]=TEXT("");
			_sntprintf(strBankerTotalScore,CountArray(strBankerTotalScore),TEXT("%I64d"),pUserData->lScore);
			...
			strBankerTime.Format( "%I64d", m_lBankerScore );
			...
			strBankerScore.Format( "%d", m_cbBankerTime );
	...
	{
		//变量定义
		RETYPELONG lScoreCount=0L;
		RETYPELONG lScoreJetton[JETTON_COUNT]={100L,1000L,10000L,100000L,500000L,1000000L,5000000L};
	...
	if (m_wMeChairID!=INVALID_CHAIR)
	{
		...
		if (wUserTimer!=0) DrawUserTimer(pDC,nWidth/2-260+75,nHeight/2-260+435-10,wUserTimer);

		
	}

#ifdef _DEBUG
	//DrawUserTimer(pDC,nWidth/2-260+75,nHeight/2-260+435-10,1);
#endif
	...
	if (m_wMeChairID!=INVALID_CHAIR)
	{
		...
		if ( pMeUserData != NULL )
		{
			...
			RETYPELONG lMeJetton = m_lMeOddScore+m_lMeEvenScore;
			_sntprintf(szGameScore,CountArray(szGameScore),TEXT("%I64d"),pMeUserData->lScore);
...
//设置信息
void CGameClientView::SetMeMaxScore(RETYPELONG lMeMaxScore)
{
...
//设置信息
void CGameClientView::SetMeOddScore(RETYPELONG lMeOddScore)
{
...
//最大下注
void CGameClientView::SetLimitScore(RETYPELONG lAreaLimitScore,RETYPELONG lUserLimitScore)
{
...
//设置信息
void CGameClientView::SetMeEvenScore(RETYPELONG lMeEvenScore)
{
...
//设置筹码
void CGameClientView::SetCurrentJetton(RETYPELONG lCurrentJetton)
{
	//设置变量
...
//历史成绩
void CGameClientView::SetHistoryScore(WORD wDrawCount,RETYPELONG lMeResultCount)
{
	//设置变量
...
//设置筹码
void CGameClientView::PlaceUserJetton(BYTE cbViewIndex, RETYPELONG lScoreCount)
{
	//效验参数
	...
	RETYPELONG lScoreIndex[JETTON_COUNT]={100L,1000L,10000L,100000L,500000L,1000000L,5000000L};
	...
	for (BYTE i=0;i<CountArray(lScoreIndex);i++)
	{
		...
		RETYPELONG lCellCount=lScoreCount/lScoreIndex[cbScoreIndex];

		//插入过虑
		if (lCellCount==0L) continue;

		//加入筹码
		for (RETYPELONG j=0;j<lCellCount;j++)
		{
			//计算位置
...
//绘画数字
void CGameClientView::DrawNumberString(CDC * pDC, RETYPELONG lNumber, INT nXPos, INT nYPos, bool bMeScore)
{
	//加载资源
	...
	//绘画桌号
	for (RETYPELONG i=0;i<lNumberCount;i++)
	{
		//绘画号码
		RETYPELONG lCellNumber=(RETYPELONG)(lNumber%10);
		if ( bMeScore )
		{
...
//绘画数字
void CGameClientView::DrawEndNumberString(CDC * pDC, RETYPELONG lNumber, INT nXPos, INT nYPos)
{
	//加载资源
	...
	//计算数目
	INT lNumberCount=0;
	RETYPELONG lNumberTemp=_Myabs64(lNumber);
	do
	{
		lNumberCount++;
		lNumberTemp/=10;
...
void CGameClientView::OnTimer(UINT nIDEvent)
{
	...
	if ( nIDEvent == IDI_OPEN_DICEBOX )
	{
		...
		if ( 15 <= m_cbOpenDiceboxIndex )
		{
			...
			SetTimer(IDI_SHOW_GAME_SCOE, 8*1000, NULL);
...
void CGameClientView::OnLButtonDown(UINT nFlags, CPoint Point)
{
	if (m_lCurrentJetton!=0L)
	{
		...
		case ID_ODD:
			{
				RETYPELONG lMaxOddScore = GetMaxOddScore();
		...
		case ID_EVEN:
			{
				RETYPELONG lMaxEvenScore = GetMaxEvenScore();
		...
		case ID_ODD:
			{
				...
				if ( lMaxOddScore < m_lCurrentJetton )
				{
					...
					if(pMeUserData->lScore<m_lCurrentJetton)
					{
						SetCursor(LoadCursor(NULL,IDC_NO));
						return TRUE;
...
//庄家信息
void CGameClientView::SetBankerInfo( WORD wChairID, BYTE cbBankerTime, RETYPELONG lScore )
{
	//设置变量
...
//庄家金币
void CGameClientView::SetBankerTreasure(RETYPELONG lBankerTreasure)
{
...
//最大下注
RETYPELONG CGameClientView::GetMaxOddScore()
{
	if ( m_wCurrentBankerChairID == INVALID_CHAIR ) return 0;

	//其他区域
	RETYPELONG lOtherAreaScore = m_lAllEvenScore;

	//最大下注
	RETYPELONG lMaxOddScore = lOtherAreaScore + m_lBankerTreasure;
...
//最大下注
RETYPELONG CGameClientView::GetMaxEvenScore()
{
	if ( m_wCurrentBankerChairID == INVALID_CHAIR ) return 0;

	//其他区域
	RETYPELONG lOtherAreaScore = m_lAllOddScore;

	//最大下注
	RETYPELONG lMaxEvenScore = lOtherAreaScore + m_lBankerTreasure;
...
//成绩设置
void CGameClientView::SetGameScore(RETYPELONG lMeCurGameScore, RETYPELONG lMeCurGameReturnScore, RETYPELONG lBankerCurGameScore)
{
...
{
	AfxGetMainWnd()->SendMessage(IDM_MANAGE_CONTROL,0,0);
}
