...
#define IDM_SEND_ONE_CARD_FINISH	WM_USER+109							//发牌完成
//////////////////////////////////////////////////////////////////////////
//发牌子项
struct tagSendCardItem
{
	CPoint           ptStartPoint;                     //动画开始的位置
	CPoint           ptTargetPoit;                     //动画结束的位置

	WORD			  wViewChairID;						   //发牌用户
	BYTE			  cbCardData;							//发牌数据

	bool             bHeadCard;                        //是从头拿的还是从尾拿的
};
//数组说明
...
public:
	...
	CImageList		   *m_pMoveCardImage;			          //显示移动时的图像列表
	WORD				m_wMinusHeadCount;					//头部空缺
	WORD				m_wMinusLastCount;					//尾部空缺
	bool              m_bTongStart;                    //开始的发牌动画结束否
	bool				m_bCartoon;							//动画标志
	CCardControl		    m_CardControl;			//用户扑克
	//函数定义
public:
	//构造函数
	CGameClientView();
...
public:
	//发牌动画
	void DispatchUserCard(WORD wViewChairID,CPoint &ptStartPoint,CPoint &ptTargetPoit, BYTE cbCardData,bool bHeadCard);
	//创建正在移动的牌
	CImageList* CreateDragImage(CWnd* pWnd, CRect& rc,BYTE cbCardData);
	//动画判断
